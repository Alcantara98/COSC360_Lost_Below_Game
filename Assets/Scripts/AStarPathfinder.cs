...
public class AStarPathfinder : MonoBehaviour
{
    ...
    public void GoTowards(Vector2 targetPosition, float atSpeed)
    {
        ...
        if (moveIndex < moves.Count)
        {
            ...
            Vector2 difference = pos - moves[moveIndex];
            ...
            transform.Translate(Vector3.left * atSpeed * Time.deltaTime);

            if (Vector2.Distance(new Vector2(transform.position.x, transform.position.y), moves[moveIndex]) < 0.1f)
            {
                moveIndex++;
            }
        }
